...
y = np.concatenate(
    [
        d_bits[["#1", "#2", "#3"]].mean(1),
        d_nmsb[["#1", "#2", "#3"]].mean(1),
        d_hidden[["#1", "#2", "#3"]].mean(1),
    ]
)
n = np.concatenate(
    [
        [str(x) + "ib" for x in d_bits["Bits"]],
        [str(x) + "msb" for x in d_nmsb["N MSB"]],
        [str(x) + "hi" for x in d_hidden["Hidden dim"]],
    ]
)
...
axes.vlines(xcim0[-2], 0, 0.15, linestyles="dashed", alpha=0.3)
axes.hlines(1 - max(arm_ycim0), 0, 5400, linestyles="dashed", alpha=0.3)


axes.plot(
    xcim0, [1 - x for x in ycim0], "x-", color="blue", label="CIM", linewidth=2
)
axes.plot(
    xcim0_p,
    [1 - x for x in ycim0_p],
    "x-",
    color="green",
    label="CIM [17]",
    linewidth=2,
...
axes.annotate(r"$3.71\times$", xy=(25, 0.07))


axes.arrow(
    min(arm_xcim0) - (min(arm_xcim0) - xcim0_p[1]),
    0.145,
    (min(arm_xcim0) - xcim0_p[1]),
    0,
    length_includes_head=True,
    head_width=0.006,
    head_length=20,
...
axes.annotate(r"$14.67\times$", xy=(16, 0.134))

axes.set_ylim(0, 1 - 0.85)
...
fig, axes = plt.subplots(
    nrows=2,
    ncols=2,
    gridspec_kw={"width_ratios": [2, 1], "height_ratios": [1, 4]},
    sharex=False,
    figsize=(7.8, 4.8),
)  #
...
p2_r = axes[0][0].bar(
    pos1, digital_ec[4], wdith_bars, bottom=digital_ec[1], color="tab:pink"
)
p3_r = axes[0][0].bar(
    pos1,
    digital_ec[0],
    wdith_bars,
    bottom=digital_ec[1] + digital_ec[4],
    color="tab:gray",
)
p4_r = axes[0][0].bar(
    pos1,
    digital_ec[2],
    wdith_bars,
    bottom=digital_ec[1] + digital_ec[4] + digital_ec[0],
    color="tab:olive",
)
p5_r = axes[0][0].bar(
    pos1,
    digital_ec[3],
    wdith_bars,
    bottom=digital_ec[1] + digital_ec[4] + digital_ec[2] + digital_ec[0],
    color="tab:cyan",
)
p6_r = axes[0][0].bar(
    pos1,
...
p2 = axes[0][0].bar(
    pos2, cim_ec[4], wdith_bars, bottom=cim_ec[0], color="tab:orange"
)
p3 = axes[0][0].bar(
    pos2,
    cim_ec[2],
    wdith_bars,
    bottom=cim_ec[0] + cim_ec[4],
    color="tab:green",
)
p4 = axes[0][0].bar(
    pos2,
    cim_ec[5],
    wdith_bars,
    bottom=cim_ec[0] + cim_ec[4] + cim_ec[2],
    color="tab:red",
)
p5 = axes[0][0].bar(
    pos2,
    cim_ec[3],
    wdith_bars,
    bottom=cim_ec[0] + cim_ec[4] + cim_ec[2] + cim_ec[5],
    color="tab:purple",
)
p6 = axes[0][0].bar(
    pos2,
    cim_ec[1],
    wdith_bars,
    bottom=cim_ec[0] + cim_ec[4] + cim_ec[2] + cim_ec[5] + cim_ec[3],
    color="tab:brown",
)
...
p1 = axes[0][1].bar(1, cim_ec[0], 0.5, color="tab:blue")
p2 = axes[0][1].bar(1, cim_ec[4], 0.5, bottom=cim_ec[0], color="tab:orange")
p3 = axes[0][1].bar(
    1, cim_ec[2], 0.5, bottom=cim_ec[0] + cim_ec[4], color="tab:green"
)
p4 = axes[0][1].bar(
    1,
    cim_ec[5],
...
p1 = axes[1][1].bar(1, cim_ec[0], 0.5, color="tab:blue")
p2 = axes[1][1].bar(1, cim_ec[4], 0.5, bottom=cim_ec[0], color="tab:orange")
p3 = axes[1][1].bar(
    1, cim_ec[2], 0.5, bottom=cim_ec[0] + cim_ec[4], color="tab:green"
)
p4 = axes[1][1].bar(
...
axes[0][1].set_ylim(0.1268, 0.1279)
axes[1][1].set_ylim(0, 0.00248)

axes[0][0].set_ylim(1.57, 1.86)
axes[1][0].set_ylim(0, 0.31)
...
leg1 = axes[1][1].legend(
    reversed((p1[0], p2[0], p3[0], p4[0], p5[0], p6[0])),
    reversed(
        (
            "DAC (25 pJ)",
            "Add (36 pJ)",
...
leg2 = axes[1][1].legend(
    reversed((p1_r[0], p2_r[0], p3_r[0], p4_r[0], p5_r[0], p6_r[0])),
    reversed(
        (
            "Sigmoid (420 pJ)",
            "IFMAP (54.75 nJ)",
...
d = 0.10  # how big to make the diagonal lines in axes coordinates
kwargs = dict(transform=axes[0][0].transAxes, color="k", clip_on=False)
axes[0][0].plot(
    (-d, +d), (0.0, 0.0), **kwargs, linewidth=2
)  # top-left diagonal
