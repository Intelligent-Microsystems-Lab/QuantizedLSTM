...
class LSTMLayer(nn.Module):
    ...
    def forward(self, input, state):
        ...
        w_mask = torch.bernoulli(
            torch.ones_like(self.cell.weight_hh) * (1 - self.drop_p)
        )
...
class QuantFunc(torch.autograd.Function):
    ...
    def forward(ctx, x, bits, x_range):
        ...
        if len(x_range) > 1:
            ...
            x01 = torch.clamp(
                x_scaled, -1 + (1.0 / step_d), 1 - (1.0 / step_d)
            )
...
def pact_af(x, a):
    if not pact_a:
        return x
    return (
        torch.sign(x) * 0.5 * (torch.abs(x) - torch.abs(torch.abs(x) - a) + a)
    )
...
class CustomMM_bmm(torch.autograd.Function):
    ...
    def forward(ctx, input, weight, bias, nl, wb, bias_r):
        ...
        wq = quant_pass(weight, wb, torch.tensor([1.0]).to(input.device))
        bq = quant_pass(bias, wb, torch.tensor([1.0]).to(input.device))
    ...
    def backward(ctx, grad_output):
        ...
        return (
            grad_input,
            grad_weight.permute(0, 2, 1),
            grad_bias.unsqueeze(1),
            None,
            None,
            None,
        )
...
class LSTMCellQ_bmm(nn.Module):
    def __init__(
        ...
        self.weight_ih = nn.Parameter(
            torch.randn(n_blocks, input_size, 4 * hidden_size)
        )
        self.weight_hh = nn.Parameter(
            torch.randn(n_blocks, hidden_size, 4 * hidden_size)
        )
        ...
        self.a1 = nn.Parameter(
            torch.tensor([128.0] * n_blocks), requires_grad=pact_a
        )
        self.a2 = nn.Parameter(
            torch.tensor([16.0] * n_blocks), requires_grad=pact_a
        )
        self.a3 = nn.Parameter(
            torch.tensor([1.0] * n_blocks), requires_grad=pact_a
        )
        self.a4 = nn.Parameter(
            torch.tensor([1.0] * n_blocks), requires_grad=pact_a
        )
        self.a5 = nn.Parameter(
            torch.tensor([1.0] * n_blocks), requires_grad=pact_a
        )
        self.a6 = nn.Parameter(
            torch.tensor([1.0] * n_blocks), requires_grad=pact_a
        )
        self.a7 = nn.Parameter(
            torch.tensor([4.0] * n_blocks), requires_grad=pact_a
        )
        self.a8 = nn.Parameter(
            torch.tensor([1.0] * n_blocks), requires_grad=pact_a
        )
        self.a9 = nn.Parameter(
            torch.tensor([4.0] * n_blocks), requires_grad=pact_a
        )
        self.a10 = nn.Parameter(
            torch.tensor([1.0] * n_blocks), requires_grad=pact_a
        )
        self.a11 = nn.Parameter(
            torch.tensor([4.0] * n_blocks), requires_grad=pact_a
        )

        self.a12 = nn.Parameter(
            torch.tensor([4.0] * n_blocks), requires_grad=pact_a
        )
        self.a13 = nn.Parameter(
            torch.tensor([4.0] * n_blocks), requires_grad=pact_a
        )
        self.a14 = nn.Parameter(
            torch.tensor([4.0] * n_blocks), requires_grad=pact_a
        )
    ...
    def forward(self, input, state, w_mask):
        ...
        if input.shape[0] == self.n_blocks:
            part1 = CustomMM_bmm.apply(
                quant_pass(pact_a_bmm(input, self.a1), self.ib, self.a1),
                self.weight_ih,
                self.bias_ih,
                self.noise_level,
                self.wb,
            ...
            part1 = CustomMM_bmm.apply(
                quant_pass(
                    pact_a_bmm(input.repeat(self.n_blocks, 1, 1), self.a1),
                    self.ib,
                    self.a1,
                ),
                self.weight_ih,
        ...
        part2 = CustomMM_bmm.apply(
            quant_pass(pact_a_bmm(hx, self.a11), self.ib, self.a11),
            self.weight_hh * w_mask,
            self.bias_hh,
            self.noise_level,
            self.wb,
            self.bias_r,
        ...
        gates = quant_pass(
            pact_a_bmm(
                quant_pass(pact_a_bmm(part1, self.a12), self.abMVM, self.a12)
                + quant_pass(
                    pact_a_bmm(part2, self.a13), self.abMVM, self.a13
                ),
                self.a14,
            ),
            self.abNM,
        ...
        #
        forget_gate_out = quant_pass(
            pact_a_bmm(torch.sigmoid(f), self.a3), self.abNM, self.a3
        )
        input_gate_out = quant_pass(
            pact_a_bmm(torch.sigmoid(i), self.a4), self.abNM, self.a4
        )
        activation_out = quant_pass(
            pact_a_bmm(torch.tanh(j), self.a5), self.abNM, self.a5
        )
        output_gate_out = quant_pass(
            pact_a_bmm(torch.sigmoid(o), self.a6), self.abNM, self.a6
        )

        #
        gated_cell = quant_pass(
            pact_a_bmm(cx * forget_gate_out, self.a7), self.abNM, self.a7
        )
        activated_input = quant_pass(
            pact_a_bmm(input_gate_out * activation_out, self.a8),
            self.abNM,
            self.a8,
        )
        new_c = quant_pass(
            pact_a_bmm(gated_cell + activated_input, self.a9),
            self.abNM,
            self.a9,
        )
        activated_cell = quant_pass(
            pact_a_bmm(torch.tanh(new_c), self.a10), self.abNM, self.a10
        )
        new_h = quant_pass(
            pact_a_bmm(activated_cell * output_gate_out, self.a11),
            self.abNM,
            self.a11,
        )
...
class LinLayer_bmm(nn.Module):
    def __init__(
        ...
        self.weights = nn.Parameter(
            torch.randn(int(n_blocks), int(inp_dim), int(out_dim))
        )
...
class KWS_LSTM_bmm(nn.Module):
    def __init__(
        ...
        # final FC layer
        self.finFC = LinLayer_bmm(
            self.hidden_dim,
            np.ceil(output_dim / n_msb),
            noise_level,
            abMVM,
            ib,
            wb,
    ...
    def get_a(self):
        return (
            torch.cat(
                [
                    self.lstmBlocks.cell.a1,
                    self.lstmBlocks.cell.a3,
                    self.lstmBlocks.cell.a2,
                    self.lstmBlocks.cell.a4,
                    self.lstmBlocks.cell.a5,
                    self.lstmBlocks.cell.a6,
                    self.lstmBlocks.cell.a7,
                    self.lstmBlocks.cell.a8,
                    self.lstmBlocks.cell.a9,
                    self.lstmBlocks.cell.a10,
                    self.lstmBlocks.cell.a11,
                    self.lstmBlocks.cell.a12,
                    self.lstmBlocks.cell.a13,
                    self.lstmBlocks.cell.a14,
                    self.finFC.a1,
                    self.finFC.a2,
                ]
            )
...
class KWS_LSTM_cs(nn.Module):
    def __init__(
        ...
        self.a2 = nn.Parameter(
            torch.tensor([16.0 * n_msb]), requires_grad=pact_a
        )
    ...
    def forward(self, inputs):
        ...
        # quantize this
        output = quant_pass(
            pact_a_bmm(self.finFC(lstm_out[-1, :, :, :]).sum(0), self.a2),
            self.abNM,
            self.a2,
        )
    ...
    def cosine_sim(self):
        ...
        weights = torch.cat(
            [
                self.lstmBlocks.cell.weight_ih.reshape(self.n_msb, -1),
                self.lstmBlocks.cell.weight_hh.reshape(self.n_msb, -1),
                self.lstmBlocks.cell.bias_ih.reshape(self.n_msb, -1),
                self.lstmBlocks.cell.bias_hh.reshape(self.n_msb, -1),
                self.finFC.weights.reshape(self.n_msb, -1),
                self.finFC.bias.reshape(self.n_msb, -1),
            ],
            dim=1,
        )
...
class KWS_LSTM_mix(nn.Module):
        ...
        if self.n_msb == 12:
            ...
            output = torch.stack(
                [
                    ...
                    output[0, :, 1] + output[2, :, 1],
                    output[0, :, 2] + output[2, :, 2],
                    output[0, :, 3] + output[2, :, 3],
                    output[0, :, 4] + output[2, :, 4],
                    output[0, :, 5] + output[2, :, 5],
                    output[1, :, 0] + output[3, :, 0],
                    output[1, :, 1] + output[3, :, 1],
                    output[1, :, 2] + output[3, :, 2],
                    output[1, :, 3] + output[3, :, 3],
                    output[1, :, 4] + output[3, :, 4],
                    output[1, :, 5] + output[3, :, 5],
                ],
                0,
            ).t()
        elif self.n_msb == 1:
    ...
    def cosine_sim(self):
        ...
        weights = torch.cat(
            [
                self.lstmBlocks.cell.weight_ih.reshape(cols, -1),
                self.lstmBlocks.cell.weight_hh.reshape(cols, -1),
                self.lstmBlocks.cell.bias_ih.reshape(cols, -1),
                self.lstmBlocks.cell.bias_hh.reshape(cols, -1),
                self.finFC.weights.reshape(cols, -1),
                self.finFC.bias.reshape(cols, -1),
            ],
            dim=1,
        )
