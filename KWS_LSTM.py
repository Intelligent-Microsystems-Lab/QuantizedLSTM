...
# general config
parser.add_argument(
    "--random-seed", type=int, default=80085, help="Random Seed"
)
parser.add_argument(
    "--method",
    type=int,
    default=1,
    help="Method: 0 - blocks, 1 - orthogonality, 2 - mix",
)
parser.add_argument(
    "--dataset-path-train",
    type=str,
    default="data.nosync/speech_commands_v0.02",
    help="Path to Dataset",
)
parser.add_argument(
    "--dataset-path-test",
    type=str,
    default="data.nosync/speech_commands_test_set_v0.02",
    help="Path to Dataset",
)
parser.add_argument(
    "--word-list",
    ...
    help="Keywords to be learned",
)
parser.add_argument("--batch-size", type=int, default=100, help="Batch Size")
parser.add_argument(
    "--training-steps",
    type=str,
    default="10000,10000,200",
    help="Training Steps",
)  # ,10000,10000 ; ,10000
parser.add_argument(
    "--learning-rate",
    type=str,
    default="0.002,0.0005,0.00008",
    help="Learning Rate",
)  # ,0.0001,0.00002 ; ,0.00008
parser.add_argument(
    "--finetuning-epochs",
    type=int,
    default=10000,
...
parser.add_argument(
    ...
    type=int,
    default=8,
    help="Number Workers Dataloader",
)
parser.add_argument(
...
parser.add_argument(
    "--testing-percentage", type=int, default=10, help="Testing Set Percentage"
)
parser.add_argument(
...
parser.add_argument(
    ...
    type=int,
    default=0,
    help="Whether to use the canoncial test data (0 non canoncial, "
    "1 canoncial.",
)
...
parser.add_argument(
    "--win-length", type=int, default=641, help="Window size in ms"
)  # 640
parser.add_argument(
    "--hop-length",
    type=int,
...
parser.add_argument(
    "--hidden", type=int, default=108, help="Number of hidden LSTM units"
)
parser.add_argument(
    "--n-mfcc",
    type=int,
    default=40,
    help="Number of mfc coefficients to retain",
)  # 40 before
...
parser.add_argument(
    "--l2", type=float, default=0.01, help="Strength of L2 norm"
)
parser.add_argument(
    "--n-msb", type=int, default=4, help="Number of blocks available"
)
parser.add_argument(
    "--cs",
    type=float,
    default=0.1,
    help="Strength cosine similarity penalization",
)

parser.add_argument("--max-w", type=float, default=0.2, help="Maximumg weight")
parser.add_argument(
    "--drop-p", type=float, default=0.125, help="Dropconnect probability"
)
parser.add_argument(
    "--pact-a",
    type=int,
    default=1,
    help="Whether scaling parameter is trainable (1:on,0:off)",
)

parser.add_argument(
    "--rows-bias", type=int, default=6, help="How many rows for the bias"
)
...
parser.add_argument(
    ...
    type=int,
    default=2,
    help="Fox mixed method, how many parallel blocks",
)
...
mfcc_cuda = torchaudio.transforms.MFCC(
...
speech_dataset_train = SpeechCommandsGoogle(
    ...
    epoch_list[-1],
    device,
    args.background_volume,
    args.background_frequency,
...
if not args.canonical_testing:
    speech_dataset_test = SpeechCommandsGoogle(
        ...
        args.unknown_percentage,
        0.0,
        non_canonical_test=not args.canonical_testing,
    )
else:
    speech_dataset_test = SpeechCommandsGoogle(
...
if args.method == 0:
    model = KWS_LSTM_bmm(
        ...
        output_dim=len(args.word_list),
        device=device,
        wb=args.quant_w,
        abMVM=args.quant_actMVM,
        abNM=args.quant_actNM,
        ib=args.quant_inp,
        noise_level=0,
        drop_p=args.drop_p,
        n_msb=args.n_msb,
        pact_a=args.pact_a,
        bias_r=args.rows_bias,
    )
elif args.method == 1:
    model = KWS_LSTM_cs(
        input_dim=args.n_mfcc,
    ...
    raise Exception(
        "Unknown method: Please use 0 for quantized LSTM blocks or 1 for bit "
        "splitting."
    )
...
for e, (x_data, y_label) in enumerate(
    islice(train_dataloader, epoch_list[-1])
):
    model.set_noise(0)
    model.set_drop_p(args.drop_p)
    if e in epoch_list:
    ...
    loss_val += (
        args.l2 * torch.norm(model.get_a()) + args.cs * model.cosine_sim()
    )
    ...
    if (e % 100 == 0) or (e == epoch_list[-1] - 1):
        ...
        for val_e, (x_vali, y_vali) in enumerate(validation_dataloader):
            ...
            output = model(x_data)
            temp_list.append(
                (output.argmax(dim=1) == y_label).float().mean().item()
            )
...
start_time = time.time()
for e, (x_data, y_label) in enumerate(
    islice(train_dataloader, args.finetuning_epochs)
):
    model.set_noise(args.noise_injectionT)
    model.set_drop_p(0)
    # train
